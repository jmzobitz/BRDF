...
gsvd_norm<-function(gsvdResult,lambda_df,rho) {
  ...
  for (j in seq_along(lambda)) {
    ...
    if ( r <= m) {
      ...
      for (i in (k+1):(k+l)) { epsilon[i]<- (1-filter3[i])*drop(t(U[,i])%*%rho_curr)}
      for (i in (k+l+1):m) { epsilon[i]<- drop(t(U[,i])%*%rho_curr)}
    } else {
      for (i in 1:(m-k)) {
        Bf[i] = filter2[i]*drop(t(U[,i])%*%rho_curr)
      }
      for (i in (k+1):m) { epsilon[i]<- (1-filter3[i])*drop(t(U[,i])%*%rho_curr)}
    }
