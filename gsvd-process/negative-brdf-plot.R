...
ls_results_renamed <- ls_results %>%
  ...
  gather(key=kernel,value=LS,"0","1","2")
...
negative_plot <- summarized_data %>%
  ...
  scale_x_discrete(limits=rev(c("0","3","4","2")),
                   labels=rev(c("LS > 0; GSVD > 0", "LS = 0; GSVD > 0", "LS > 0; GSVD = 0", "LS = 0 ; GSVD < 0"))) + coord_flip() +
  scale_fill_discrete(name="Kernel Weights",
                      labels=c(as.expression(bquote(f[iso])),as.expression(bquote(f[vol])),as.expression(bquote(f[geo])))) +
